    ...
    public static class Easing
    {
        ...
        public static class Expo
        {
            ...
            public static float InOut(float n)
                ...
                : (2 - Mathf.Pow(2.0f, -20.0f * n + 10.0f)) / 2.0f;
        }
        ...
        static readonly Func<float, float>[] EasingFunctions = new Func<float, float>[]
        {
            Sine.In,
            Sine.Out,
            Sine.InOut,
        ...
        /// and `y_end`.
        /// </summary>
        /// <param name="x">x The value to be normalized.</param>
        /// <param name="x_min">x_min The minimum value of the range to normalize `x` to.</param>
        /// <param name="x_max">x_max The maximum value of the range to normalize `x` to.</param>
        /// <param name="y_start">y_start The starting value of the output range.</param>
        /// <param name="y_end">y_end The ending value of the output range.</param>
        ...
        public static float NormalizedEasing<T>(T easingType, float x, float x_start, float x_end, int start_displacement, int end_displacement)
            where T : struct, Enum
            => Normalize(
                EasingFunctions[Convert.ToInt32(easingType)].Invoke(Normalize(x, x_start, x_end))
                , 0.0f
                , 1.0f
                , start_displacement
                , end_displacement);
